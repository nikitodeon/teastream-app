...
export class StreamService {
	...
	public async findRandom() {
		...
		if (total === 0) return [] // Если стримов нет — возвращаем пустой массив

		const count = Math.min(4, total) // Берем минимум из 4 и общего количества стримов

		const randomIndexes = new Set<number>()

		while (randomIndexes.size < count) {
			randomIndexes.add(Math.floor(Math.random() * total)) // Добавляем случайные индексы
		}

		const streams = await this.prismaService.stream.findMany({
			where: { user: { isDeactivated: false } },
			include: { user: true, category: true },
			take: total, // Загружаем все стримы, чтобы выбирать по индексам
			skip: 0
		})
